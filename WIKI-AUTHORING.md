...
- **У таблицях** — іконка **inline** в клітинці з назвою: `| <img src="…" alt="Назва" width="36"> **Назва** | … |`. Еталон: `crime/weed.md`, `crime/heisenberg.md`.
- **Не** роби **порожню колонку** під іконку (`| | Предмет |`) — GitBook перетворює таку таблицю на database block і **додає рядок пошуку** над нею.
- **Не** `data-size="line"` у таблицях — ~16px, ледь видно. **Не** `data-size="original"` / голий `![](path)` — розтягує клітинку.
- Шлях лише через `.gitbook/assets/`, не `growing/` чи інші підпапки.
- **Поза таблицею** (списки, кроки рецепту) — `width="36"` або `data-size="line"` для дрібних згадок у тексті.
