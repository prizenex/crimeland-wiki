...
- **Таблиці з предметами** — **окрема колонка під іконку** (порожній заголовок `| |`, як у `business/overview.md`): іконка лише в першій клітинці `width="36"`, назва — у **наступній** колонці. Так GitBook рендерить іконки **великими**; якщо зліпити іконку з назвою в одну клітинку — вони стають дрібними inline.
