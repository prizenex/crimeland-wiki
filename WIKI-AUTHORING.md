...
- **Таблиці з предметами** — **окрема колонка під іконку** (порожній заголовок; GitBook покаже як «Text»), **лише** `width="36"` у клітинці іконки, назва в **наступній** колонці. Не плутати з `data-size="line"` — той лише для списків/рецептів. Еталон: `crime/weed.md`, `crime/heisenberg.md`.
- GitBook перетворює таку таблицю на **database block** із пошуком — це нормально для списків предметів/сортів.
