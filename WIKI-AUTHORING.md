...
- **У таблицях іконок предметів немає** — лише **назва жирним** (`**Горщик**`, `**Амоніак**`). У GitBook `data-size="line"` у клітинці все одно ледь видно.
- Іконки предметів — у **тексті поза таблицею** (списки, кроки, hint): `<img src="..." alt="Назва" data-size="line">` поруч із назвою, або без картинки.
- **Ніколи** `![](path)` / `data-size="original"` для предметів у таблицях — рендериться на всю клітинку.
