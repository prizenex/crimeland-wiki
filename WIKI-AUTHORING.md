...
- **У таблицях** — `<img src="../.gitbook/assets/<item>.png" alt="Назва" width="36">` **перед** жирною назвою. Шлях лише через `.gitbook/assets/`, не `growing/` чи інші підпапки.
- **Поза таблицею** (списки, кроки) — `data-size="line"` або `width="36"`.
- **Не використовуй** `data-size="line"` у таблицях — ледь видно. **Не використовуй** `data-size="original"` / голий `![](path)` у таблицях — розтягує клітинку.
