...
- **Маркер:** не дублюй клавішу з підказки в грі — опиши дію та місце взаємодії.
- **Таргет без маркера:** згадуй **`E` / `лівий Alt`** лише коли скрипт для цієї взаємодії справді вмикає таргет. За потреби один раз додай посилання на [Керування](../basics/controls.md). **Не згадуй** назву ресурсу таргета.
...
- **Таблиці з предметами** — **окрема колонка під іконку** з заголовком **`| Іконка |`** (не порожній `| |` — GitBook перетворює таку таблицю на database block і показує **пошук** над нею). Іконка лише в першій клітинці `width="36"`, назва — у **наступній** колонці. Так іконки **великі**; якщо зліпити з назвою в одну клітинку — дрібні inline.
- **Не** `data-size="line"` у колонці іконок таблиці (~16px). **Не** `data-size="original"` / голий `![](path)` — розтягує клітинку.
- **Списки, кроки рецепту, згадки в реченні** — лише `data-size="line"` (дрібна inline-іконка в тексті). Еталон: етапи варки в `crime/heisenberg.md`.
- Шлях лише через `.gitbook/assets/`, не `growing/` чи інші підпапки.
- Іконки мапи (`../.gitbook/assets/blips/{id}.png`) у таблицях **можна** — вони дрібні й упізнавані.
...
1. **Вступ із користю** — починай з того, що гравець хоче зробити або дізнатися, а не з визначення механіки. 1–3 живі речення без повтору назви сторінки.
2. **Сценарій дії** — пояснюй природною мовою: де гравець починає, що бачить і який результат отримає. Нумерований список залишай лише для дій, де важливі порядок або розгалуження.
3. **Таблиці** — лише коли без них гірше (див. нижче)
4. **Предмети з іконками** — inline `data-size="line"` у тексті; у каталогах — колонка `| Іконка |`
5. **`{% hint style="info" %}`** — один важливий нюанс саме після дії, якої він стосується; не збирай випадкові поради в розділ наприкінці й не дублюй абзац над ним

### Перед тим як писати

Сформулюй одне запитання, з яким гравець відкрив сторінку: «де це знайти?», «що взяти із собою?», «скільки це коштує?» або «чому дія не спрацьовує?». Перший екран сторінки має дати відповідь або привести до неї.

Подавай інформацію в порядку реального досвіду: **куди піти → що побачить гравець → що зробити → що отримає або ризикує втратити**. Для довгих механік після вступу додай короткий блок «Що знадобиться» чи «Коротко», але тільки з даними, які справді допомагають прийняти рішення.

### Коли таблиця, а коли текст

**Залишай таблицю лише якщо без неї гірше читати дані:**

- ціни, шанси %, cooldown, нагороди (еталон: `crime/atms.md`)
- порівняння колонками (мала / велика ювелірка, Fleeca / Pacific)
- каталог луту / предметів (багато рядків, іконки, `$`)

**Усе інше — текст:**

- будь-яка двоколонкова «мітка → пояснення» (не лише «Параметр / Деталі» — **усі** такі шаблони)
- кроки (`Крок | Дія`) → нумерований список
- правила, вимоги, «де знайти», оплата з 2–4 пунктів
- статуси, режими, поради — абзац або `- **мітка** — пояснення`

> **Правило на око:** таблиця з ≤4 рядків без `$` / `%` / порівняння колонок — майже завжди зайва. Скрипт `tools/dedupe_tables.py` допомагає масово, але автор **перечитує** сторінку вголос — якщо звучить як Excel, перепиши prose.

### Мова (не «машинний» текст)

- Player-facing текст: **українська**, як пояснив би друг у голосовому, не як ТЗ
- Без вульгаризмів
- Конкретні числа з конфігу, не «приблизно» / «багато»
- **Без тавтології:** не повторюй у hint те, що вже в абзаці вище; не пиши «на сервері є можливість» замість «можна»
- Одне речення = одна думка; прибирай вступи на кшталт «даний розділ розповідає про…»
- Не перетворюй кожен клік на окремий пункт. «Відкрийте каталог, оберіть малюнок і приміряйте його» читабельніше за три формальні команди, якщо порядок не критичний.
- **Не описуй процес ланцюжком `→`.** `клієнт → планшет → рахунок → оплата` — це блок-схема, не гайд; гравець напружує мозок, щоб «зібрати» речення. Пиши **хто що робить і навіщо** звичайними реченнями. Стрілка `→` лишається лише для: короткого шляху в UI (`Tab` → ПКМ → **Використати**), математики цін (`25$` → `85$` → `60$` чистими), діапазонів класів (`B` → `S+`). Якщо в рядку більше двох `→` або це цілий сценарій — перепиши prose.
- Не вигадуй категорії та причини цін на кшталт «рідкісні предмети дорожчі». Якщо механіка не підтверджена конфігом, скажи лише те, що гравець точно побачить: наприклад, остаточну суму перед оплатою.
- Не починай із «У цьому розділі ви дізнаєтесь…», «на сервері є можливість…» або повтору заголовка. Це метатекст автора, а не користь для гравця.
- Не називай приблизні значення точними й не маскуй невідоме словами «можливо», «інколи» чи «зазвичай». Перевір факт або поясни, де гравець побачить актуальне значення.
- Не зловживай англіцизмами з конфігу: `cooldown` → «час очікування», `Dispatch` → «виклик для поліції», якщо назва системи не показується гравцеві буквально.
- Заголовок + список краще, ніж таблиця «Параметр | Значення» для очевидних речей
...
**Обов’язково:** кожну важливу **локацію на мапі** в статті (паркінг, магазин, штрафмайданчик, робота, зона активності) супроводжуйте відповідною іконкою з `.gitbook/assets/blips/`. Звіряйте її з `sprite` у конфігу (`SetBlipSprite` / `blip.id`), щоб гравець упізнав позначку на мапі (`P`).

Ставте іконку поруч з описом локації, щоб гравець упізнав її на мапі.
...
## 5. Раніше захищені сторінки

`basics/phone.md`, `crime/atms.md`, `crime/weed.md`, `crime/twenty-four-seven.md`, `crime/contract-tablet.md`, `crime/houses.md`, `crime/jewelry-store.md`, `crime/car-theft.md`, `crime/containers.md` — раніше мали статус «не чіпати без запиту». Наразі цей статус знято: стилістичний прохід гайдбука (жовтень 2026) торкнувся й цих сторінок нарівні з іншими. Якщо потрібно знову «заморозити» якусь сторінку — додай її сюди явно.
...
- [ ] Немає таблиць «Параметр / Деталі» на 2–4 рядки — простий текст або список
