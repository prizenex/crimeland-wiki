...
- **У таблицях** — **окрема колонка** лише для іконки (перша колонка без заголовка), назва — **наступною** колонкою жирним. Еталон: `crime/weed.md` → «Гровінг у горщику» / «Добрива».
- Синтаксис: `| | Назва | … |` + `| <img src="../.gitbook/assets/<item>.png" alt="…" data-size="line"> | **Назва** | … |`
- **Не** став іконку й текст **в одну клітинку** — GitBook роздуває картинку на всю висоту рядка.
- **Не використовуй** `width="…"`, `data-size="original"` / голий `![](path)` у таблицях.
- Шлях лише через `.gitbook/assets/`, не `growing/` чи інші підпапки.
- **Поза таблицею** (списки, кроки рецепту) — `data-size="line"` поруч із текстом.
